...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=13, fig.width=11}
...
#Load data
dat_cleaning_5 <- readRDS("Data/RData/Data_for_plotting_qualitative_pca_vars.rds")
...
Flower_sym <- function(PC, x="PC1", y="PC2") {
  ...
  plot <- plot + theme_ms() +ylim(-4,4) + xlim(-4,4) +  theme(legend.position = c(0.165, 0.1)) + ggtitle("D)") 
...
Breeding_syst <- function(PC, x="PC1", y="PC2") {
  ...
  plot <- plot + theme_ms() +ylim(-4,4) + xlim(-4,4) +  theme(legend.position = c(0.163, 0.115)) + ggtitle("F)") 
...
library(cowplot)
 plot_grid(Compatibility(PC),Life_form(PC),Life_span(PC),Flower_sym(PC),Flower_sha(PC),Breeding_syst(PC), ncol=2,nrow=3,  align = "v")
