...
*Hypothesis*
[overall, guay, pero le falta story telling]()
...
3) Can we predict the diversity of pollinator interactions with the phylogenetic distance of a community? -Hypothesis- communities with a higher number o close related species will likely have trait redundancy and less diversity of interactions.
[IB: Este ultimo punto es interesante, PERO quizas es otro paper? Mirate https://www.nature.com/articles/nature05956 Rezende tiene otros papers al respecto]()

#MATERIALS AND METHODS

This research has collated networks from 25 different studies from very different geographical regions across the world. All of these networks are from natural habitats or systems with a minor human intervention. The nature of these networks is both weighted (N) and unweighted (N) with the former just with visitation data which could serve as proxy of pollination with pollinator efficiency as limitation (REF). These studies have been counducted with different sampling methodologies and sampling effort and our aim is not to compare them but to get a general picture of how the selected functional traits shape these networks and the different taxonomic groups. The networks of these studies that were done in consecutive years or in areas close to each other with a similar species compositon were combined in metawebs. This makes a total of 30 different unique networks/metawebs when all are considered by their maximum level of organization (metaweb if possible). All the species names were searched with the help of the package taxsize [@chamberlain2013taxize] from two different data sources ncbi and itis, for plants and floral visitors respectively. We filled the not found searches manually with the help of <http://www.theplantlist.org/> and <http://www.mobot.org/> for plants and  <http://www.catalogueoflife.org/> for floral visitors.


For all of these studies we have done an extensive literature and online search of different floral traits. These traits are breeding system, compatibility system, selfing rate (with fruit and seed set), flower shape, flower symmetry, flower size (width and length), style length and ovule number. Moreover, we also recorded plant height, life span and life form for these species.
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=7, fig.width=8, fig.cap="Percentage of species of each order per network/metaweb (qualitative data)."}
library(dplyr)
library(tidyr)
library(plyr)
library(ggplot2)
...
la <- c("Lundgren 2005", "Dupont 2009 b", "Kaiser-Bunbunry 2014",  "Primack 1983 a", "Bek 2006", "Kevan 1970","Traveset 2013", "Kaiser-Bunbunry 2009",  "Kato 2000", "Primack 1983 b", "Primack 1983 c", "Elberling 1999", "Dicks 2002", "Olesen 2002 a", "Mcmullen 1993", "Fang 2012", "Olesen 2002 b", "Dupont 2009 a", "Inouye 1988", "Bundgaard 2003","Souza 2018", "Inouye 1990", "Small 1976", "Robertson 1929", "Bartomeus 2008 b", "Bartomeus 2008 a", "Chacoff 2011","Bartomeus unp. 2015","Ramirez 1992","Ramirez 1989")
...
# IB: No me cuadra la cantidad de dipteros (y pocos hymenopteros) en mis redes, recheck colores. Ojo, quizas esta bien, pero me sorprende. 
# IB; Para visitas hay oalgo mal seguro, no hay abejas en 2015 ¿?
#ARREGLADO, las nombres de las redes estaban mal ordenados
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=7, fig.width=8, fig.cap="Percentage of visits of each order per network/metaweb (quantitative data)."}
library(dplyr)
library(tidyr)
library(plyr)
library(ggplot2)
...
la <- c("Lundgren 2005", "Elberling 1999", "Kaiser-Bunbunry 2009", "Kato 2000", "Dupont 2009", "Olesen 2002 a", 
       "Traveset 2013", "Bartomeus 2008 a",  "Inouye 1988", "Dicks 2002", "Inouye 1990", "Small 1976", "Bartomeus 2008 b","Chacoff 2011", "Fang 2012", "Kaiser-Bunbury 2014", "Olesen 2002 b", "Bartomeus unp. 2015", "Souza 2018")
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.align='center', fig.cap="Pie charts on the coordinates of each study site  with a classification of three main breeding systems (hermaphroditism, dioecy and monoecy)."}
library(ggplot2)
library(scatterpie)
library(sf)
...
## Molaria compararlo con estimas a nivel mundial de que % de plantas son hermafroditas, (y igual para todos los traits). En otras palabras, salen estas porporciones por que reflejan las proporciones generales? O se desvian?)  
```

\newpage


```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.align='center', fig.cap="Pie charts on the coordinates of each study site  with the compatibility level classified in three main groups (self compatible, partially self compatible and self incompatible)."}
library(ggplot2)
library(scatterpie)
library(sf)
world <- map_data('world')
...
##Que flipe que al norte hay más self-compatible?! Esto debe estar decrito en la literatura, verdad? Hay una correlación entre la diversidad de abejas y el nivel de self incompatibility?  
```
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.align='center', fig.cap="Pie charts on the coordinates of each study site  with the life forms for all the species divided in trees, shrubs, herbs and vines."}
library(ggplot2)
library(scatterpie)
library(sf)
world <- map_data('world')
...
kable(m, "latex", longtable = T, booktabs = T,linesep = "\\addlinespace", caption = "Metadata for all the different networks/metawebs used at their maximum level of organization (metaweb if possible).") %>%
  kable_styling(latex_options = c("repeat_header","striped"), font_size = 7, full_width=T)
  
### en 2008 por que hay 3 networks? Netiendo como puede haver 12, 6 o 2, pero 3? OK entendido, ya veo el 3+3! pon 2008a 2008b?
```
\end{landscape}

Now I'm going to explorea small subset of networks from Bartomeus 2015 to see how the analysis would be. Now I show graphically the networks with sankey diagrams. In first place the sankey diagram for the metaweb and then 4 panels in 4 different networks for the 16 different sites that were sampled in this project.

*Add individual chunks to add captions, think about combining panels...*
...
include_graphics("Bee_fun_x8_1.pdf", error = T)       

include_graphics("Bee_fun_x8_2.pdf", error = T)       
