...
aes(x = -PC1, y = Interaction),size = 2.5, alpha=0.4) + geom_line(size=1.5)  + ylab("Interaction")+ xlab(NULL)+
...
p2 <- ggplot(ce_pc2[[1]], aes(x = -PC2, y = (estimate__+1), group=guild, colour=guild)) + geom_point(data = dat_analysis,
  ...
  ylim(0,quantile(dat_analysis$Interaction, 0.95)) + ylab(NULL)+ xlab("Low poll. depend.   High poll. depend.")+
...
p3 <- ggplot(ce_pc3[[1]], aes(x = -PC3, y = (estimate__+1), group=guild, colour=guild)) + geom_point(data = dat_analysis,
  ...
  ylim(0,quantile(dat_analysis$Interaction, 0.95)) + ylab(NULL)+ xlab("Short styles      Long styles\n High poll. depend.   Low poll. depend.")+
  theme_ms() + theme(legend.position = "none",plot.title = element_text(size=rel(2.5),hjust = 0,face="bold"))+ 
