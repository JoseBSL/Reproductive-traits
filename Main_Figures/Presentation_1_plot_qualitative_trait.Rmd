...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=12, fig.width=12}
...
Compatibility <- function(PC, x="PC1", y="PC2") {
  ...
  plot <- plot + geom_point(data=dat, aes(-x, -y, colour=Compatibility),size=3.5)+scale_color_manual(values=c("cyan4", "deepskyblue2","orange","gray7"))
  ...
  # add arrows
  plot <- plot + geom_segment(data=datapc,linejoin="round", lineend="round",aes(x=0, y=0, xend=-v1, yend=-v2),size=2.2, arrow=arrow(length=unit(0.5,"cm")), alpha=1, color="black")
  ...
  plot <- plot + geom_segment(data=datapc, aes(x=0, y=0, xend=v1, yend=v2),size=1.4, arrow=arrow(length=unit(0,"cm")),linetype=2, alpha=0.8, color="black")
...
#                          label = PCAloadings$Variables, #color="black",size=12, fontface=2)
  
  #CHANGE THEME
  
  plot <- plot + theme_ms() +ylim(-4,4) + xlim(-4,4) +  theme(legend.position = c(0.20, 0.14),legend.title=element_text(size=24)) +ggtitle("") 
...
# #plot_grid(Compatibility(PC),Life_form(PC),Life_span(PC),Breeding_syst(PC),Flower_sha(PC),Flower_sym(PC),Nectar(PC), ncol=2,nrow=4,  align = "hv")
  ...
  plot_grid(Compatibility(PC),ncol=1,nrow=1,  align = "hv")
