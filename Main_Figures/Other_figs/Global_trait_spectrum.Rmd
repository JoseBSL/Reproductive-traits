...
all_spp <- function(PC, x="PC1", y="PC2") {
  ...
  plot <- plot + theme_bw() + annotation_raster(tree_1, xmin = -4.1, xmax = -2.85,ymin = 2.05, ymax = 4.05)+
     annotation_raster(style_l_1, xmin = 2.10, xmax = 2.90,ymin = 2.55, ymax = 3.7) +
 ...
 plot <- plot + annotate("text", x = -(PCAloadings$PC1*c(4.7,4.95,5.6,7.45,7.2,6.15)), y = -(PCAloadings$PC2*c(4.693,2.5,3.2,6.6,5,5.5)+c(0,0,0,0,0.4,0)),
                         label = PCAloadings$Variables, color="black",fontface =2,size=5)
  
 
 plot <- plot + annotate("text", x = (PCAloadings$PC1*c(4.6,4.95,4.5,6.6,7.2,5.15)), y = (PCAloadings$PC2*c(3.83,6.4,5.7,7.4,-1,7.45)+c(0,0,0,0,-0.41,0)),
                         label =c("Low selfing", "Few flowers", "Small flowers", "Short styles", "Few ovules", "Short plants" ), color="black",fontface =2,size=5)
