...
all_spp <- function(PC, x="PC1", y="PC2") {
  ...
  #  scale_alpha(guide = 'none')
    ...
    plot <- plot + geom_point(data=dat, aes(x, y),size=2.5, color="azure4")
  ...
  # add arrows
  plot <- plot + geom_segment(data=datapc,linejoin="round", lineend="round",aes(x=0, y=0, xend=-v1, yend=-v2),size=5.5, arrow=arrow(length=unit(0.5,"cm")), alpha=0.8, colour=c("black"))
...
#  rownames(PC$L) <- c("High selfing", "Many flowers", "Large flowers", "Long styles", "Many ovules", "Tall plants" )
 ...
 #plot <- plot + annotate("text", x = (PCAloadings$PC1*c(4.6,4.95,4.5,6.6,7.3,5.15)), y = (PCAloadings$PC2*c(3.83,6.4,5.7,7.4,-1,7.45)+c(0,0,0,0,-0.41,0)),
  ...
  plot <- plot + theme_ms() +ylim(-3.5,3) + xlim(-3.5,3) +  theme(legend.position = c(0.095, 0.11)) +ggtitle("") +theme(axis.line=element_blank(),axis.text.x=element_blank(),
