...
Trait <- c("Plant height (m)", "Flower width (mm)", "Flower length (mm)", "Inflorescence width (mm)", "Style length (mm)", "Ovules per flower",
           "Flowers per plant", "Nectar ($\\mu$l)", "Nectar (mg)", "Nectar concentration ($\\%$)", "Pollen grains per flower", "Autonomous selfing")
...
dat <- data.frame( Trait, Trait1, T.cat)


colnames(dat) <- c(" ", "Traits", "Categories")


kbl(dat, booktabs = T, linesep = "\\addlinespace", escape = FALSE, align = c("l","l","l")) %>%
  kable_styling(full_width = F,font_size = 10) %>%
  add_header_above(c("Quantitative traits" = 1,"Qualitative traits" = 2),bold=T)   %>% row_spec(0:12, bold=T) %>% save_kable(file = "table_1.png",density = 700) 
