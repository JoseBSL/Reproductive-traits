...
{\centering \includegraphics[width=0.9\linewidth]{output/figures/unnamed-chunk-5-1} 

}

\caption{\textbf{Fig. S9.} Association between the main axes of trait variation (showed on different columns) and the distinct plant species level network metrics (showed on different rows). The x-axis shows the most illustrative trait extremes of the trait continuum. Note that the plots are coloured by the different network metrics.}\label{fig:unnamed-chunk-5}
\end{figure}
