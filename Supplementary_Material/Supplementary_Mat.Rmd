---
#title: \singlespacing \vspace{-2.5cm} \LARGE \textbf{Covariation among reproductive traits in flowering plants shape their interactions with pollinators}
...
## Covariation among reproductive traits in flowering plants shape their interactions with pollinators
...
#LABELS
label <- c("Autonomous selfing", "Flower number", "Inflorescence width" ,"Flower width", "Flower length", "Style length", "Ovule number", "Plant height", "Nectar per flower ($\\mu$l)","Pollen grains per flower")
#LAMBDA
lambda <- c(selfing$lambda , flower_n$lambda, inflow_width$lambda, flower_width$lambda, flower_length$lambda, style_n$lambda, ovule_n$lambda, plant_height$lambda, nectar_ul$lambda, pollen$lambda)
#P-VALUE
p_value <- c(selfing$P , flower_n$P, inflow_width$P,flower_width$P, flower_length$P, style_n$P, ovule_n$P, plant_height$P,nectar_ul$P, pollen$P)
...
t = t %>% select(!Autonomous_selfing_level)

colnames(t) <- c("Breeding system","Compatibility system","Autonomous selfing level", "Flower shape", "Flower symmetry", 
                 "Flower number per plant", "Inflorescence width", "Flower width", "Flower length","Style length", "Ovule number", 
                 "Life form", "Lifespan","Plant height","Nectar qualitative","Nectar per flower (microlitres)", "Nectar per flower (miligrames)",  "Pollen grains per flower")
...
all_spp <- function(PC, x="PC1", y="PC2") {
  ...
  plot <- plot + xlab(paste("Flower number - Flower size axis ", "(",(percentage[1]),"%",")", sep = "")) #XLAB
  plot <- plot + ylab(paste("Autonomous selfing - Floral display ", "(",(percentage[2]),"%",")", sep = "")) #YLAB
...
plot_apis <- readRDS("../Data/RData/Plot_apis_non_apis.rds")
...
pc3 <- function(PC, x="PC1", y="PC3") {
  ...
  plot <- plot + xlab(paste("Flower number - Flower size axis ", "(",(percentage[1]),"%",")", sep = "")) #XLAB
  plot <- plot + ylab(paste("Style length - Autnomous selfing axis ", "(",(percentage[3]),"%",")", sep = "")) #YLAB
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, out.width="85%",  fig.height=12, fig.width=18,fig.align='center',  fig.cap="\\textbf{Fig. S7.} Location in the trait space of the plants that were visited by each of the different pollinator guilds. The red square indicates that the pollinator guild has been observed in that specific plant. Note that the interactions used to represent the plant-pollinator guild connection are a subset of the total interactions considered in this study as indicated in the Network analysis section (see main text)."}
...
breed_1 <- ggplot(breed1,aes(x= Breeding_system,y= emmean,
  ...
  scale_color_manual(values=c("forestgreen","orange", "black"))+ labs(title =  "(g) Breeding system")+
  geom_text(nudge_x = c(0), nudge_y = c(0.45),color   = "black") + theme(axis.text = element_text( size = rel(0.9)))
...
breed_2 <- ggplot(breed2,aes(x= Breeding_system,y= emmean,
  ...
  scale_color_manual(values=c("forestgreen","orange", "black")) + labs(title =  "(h) Breeding system")+
  geom_text(nudge_x = c(0), nudge_y = c(0.45),color   = "black") + theme(axis.text = element_text( size = rel(0.9)))
...
comp_1 <- ggplot(comp1,aes(x= Compatibility_system,y= emmean,
  ...
  width =  0.25,size =  0.8)+ylab("Least square mean")+xlab("")+ theme(legend.position = "none")+ theme_ms()+
  scale_color_manual(values=c("cyan4", "deepskyblue2","orange","gray7")) +
    labs(title = "PC2",subtitle = "(a) Compatibility system")+
...
comp_2 <- ggplot(comp2,aes(x= Compatibility_system,y= emmean,
  ...
  width =  0.25,size =  0.8)+ylab("")+xlab("")+ theme(legend.position = "none")+ theme_ms()+
  scale_color_manual(values=c("cyan4", "deepskyblue2","orange","gray7")) + 
    labs(title = "PC2",subtitle = "(b) Compatibility system")+
  theme(plot.title = element_text(hjust=0.5,size=26, face="bold"),plot.subtitle=element_text(size=16))+
  geom_text(nudge_x = c(0), nudge_y = c(0.35),color   = "black")
...
lifeform_1 <- ggplot(lif_1,aes(x= life_form,y= emmean,
  ...
  geom_errorbar(aes(ymin  =  lower.CL,ymax  =  upper.CL,  color = life_form),
                width =  0.25,size =  0.8)+ylab("Least square mean")+xlab("")+ theme(legend.position = "none")+ theme_ms()+
  scale_color_manual(values=c("forestgreen", "darkorange2","gray7"))+ labs(title  = "(c) Life form")+
...
lifeform_2 <- ggplot(lif_2,aes(x= life_form,y= emmean,
  ...
  geom_errorbar(aes(ymin  =  lower.CL,ymax  =  upper.CL,  color = life_form),
                width =  0.25,size =  0.8)+ylab("")+xlab("")+ theme(legend.position = "none")+ theme_ms()+
  scale_color_manual(values=c("forestgreen", "darkorange2","gray7")) + labs(title =  "(d) Life form")+
  geom_text(nudge_x = c(0), nudge_y = c(0.5),color   = "black")
...
#PLOT!
comp_1 + comp_2  + lifeform_1 + lifeform_2+ lifespan_1 + lifespan_2 + breed_1 + breed_2+ shape_1 + shape_2 +
...
plot <- readRDS("../Data/RData/Plot_bee_families.rds")
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=18, fig.width=26, fig.cap="\\textbf{Fig. S10.} Association between the main axes of trait variation (showed on different columns) and the distinct plant species level network metrics (showed on different rows). The x-axis shows the most illustrative trait extremes of the trait continuum. Note that the plots are coloured by the different network metrics."}
...
plot_metrics_pcs <- readRDS("../Data/RData/plot_metrics_pcs.rds") 
