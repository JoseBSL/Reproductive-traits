---
title: \singlespacing \vspace{-2.5cm} \LARGE \textbf{Trade-offs among plant floral and reproductive traits determine interactions with floral visitors}
...
Jose B. Lanuza$^{1,2}*$, Romina Rader$^{1}$, Jamie Stavert$^{3}$, Liam K. Kendall$^{4}$, Manu E. Saunders$^{1}$ and Ignasi Bartomeus$^{2}$
...
Description of the traits compiled in this study
...
Supplementary Table S2: Loadings of the first three axes of trait variation of the phylogenetically informed principal component analysis with the full set of species

Supplementary Table S3: Loadings of the first three axes of trait variation of the phylogenetically informed principal component analysis with the subset of species with data of nectar and pollen quantity 

Supplementary Table S4: Statistical association between the different categorical variables and the first three main axes of trait variation with the full set of species
...
Supplementary Fig. S1: Percentage of present and missing values of the different traits

Supplementary Fig. S2: Phylogenetically informed principal component analysis for the species that did not have missing values

Supplementary Fig. S3: Fitted posterior estimates of the number of visits made by bees including and excluding honey bees on the main axes of trait variation
...
Supplementary Fig. S6: Phylogenetically informed principal component analysis for the subset of species with floral rewards.
...
Supplementary Fig. S8: Visitation rate of the main bee families in relation to the main axes of trait variation
...
-	Selfing level: We recorded the selfing level of the different species with both quantitative and qualitative data. The qualitative data were divided in four main categories, high selfers, medium selfers, low selfers and none which was for the species that were unable to self-pollinate. In addition, quantitative data were also divided into these four categories with the following criteria: from 0% to lower than 1% ‘none’, from 1% to 25% ‘low’, from 26% to 75% ‘medium’ and from 76% to 100%  ‘high’.
...
-	Flower morphology: We looked for images and illustrations of the flowers from the different species on the floras and available resources in order to categorize the flower shape. We divided the flowers in 8 main different categories: open, tube, campanulate, funnelform, papilionaceous, spike and capitulum. 
...
-	Flower and inflorescence size: We searched for flower length and flower and inflorescence width (mm) for all species. When possible, we measured this trait with the help of online images and the software ImageJ. 

-	Flower number per plant: We compiled information about the number of flowers per plant for all species. However, this field was rare to find and we also used online images of the different species in order to calculate rough numbers of flowers per plant. We referenced all the filled fields in order to be able to follow the images that were used for these counts. It is important to note, that these numbers are not predicted to be the exact number of flowers per plant in each species but an approximate indicator of the reproductive investment in each species that will allow for its use in macroecological analysis.
 
-	Ovule number: We searched for the number of ovules per flower for all species. The number of ovules of Asteraceae species are considered as the total number of ovules per capitulum. Many species were filled at either the genus or family level because the number of ovules in this taxonomic groups is considered to be mostly constant (e.g., Lamiaceae, Boraginaceae, or Apiaceae).
 
-	Style length: The length of the style (mm) of the different species. When possible, we calculated style length from online images and illustrations.

- Nectar provision: We recorded the presence and absence of nectar for all species. In addition, we also searched for microlitres and milligrams of nectar per flower and nectar concentration. In general terms, nectar data was rarely present and we try to minimise the loss of information by filling some species at family level. For instance, Solanaceae species are described as nectarless. Therefore, all Solanaceae species were recorded with having an ‘absence’ of nectar.

- Pollen grains per flower: We recorded pollen grains per flower for all species but this field was rarely described in the literature. When we could only find the pollen:ovule ratio, we converted it back to pollen grains by multiplying by ovule number. 

_Vegetative traits_

- Life form, life span and plant height: We divided the different plant species in 4 main categories: herbs, vines, shrubs and trees. Moreover, we also divided the species between short-lived species (annual, biennial and short-lived perennials) and perennial species (long-lived). Finally, we also searched for the average height (m) of the different species and annotated the maximum and minimum height when possible. We calculated the average between the maximum and minimum height to get an approximate average height of the species when the average was not indicated.
...
dat <- dat %>% mutate(across(is.numeric, ~ round(., 2)))
 ...
 kable(dat, longtable = T, booktabs = T,linesep = "\\addlinespace", escape=FALSE,  caption = "\\textbf{Supplementary Table S2 | Loadings of the first three axes of trait variation of the phylogenetically informed principal component analysis with the full set of species.}") %>%
...
kable(dat, longtable = T,"latex", booktabs = T,linesep = "\\addlinespace", escape=FALSE,  caption = "\\textbf{Supplementary Table S3 | Loadings of the first three axes of trait variation of the phylogenetically informed principal component analysis with the subset of species with data of nectar and pollen quantity.}") %>%
  kable_styling(latex_options = c("repeat_header","striped"), font_size = 12, full_width=F,position = "center") 
...
kable(d, longtable = T,"latex" , booktabs = T,linesep = "\\addlinespace", escape=FALSE, caption = "\\textbf{Supplementary Table S4 | Statistical association between the different categorical variables and the first three main axes of trait variation with the full set of species.}") %>%
  kable_styling(latex_options = c("repeat_header","striped"), font_size = 12, full_width=F,position = "center") 
...
kable(tabla, "latex", longtable = T, booktabs = T,linesep = "\\addlinespace", escape=FALSE,caption = "\\textbf{Supplementary Table S5 | Phylogenetic signal of the different quantitative traits.}") %>%
  kable_styling(latex_options = c("repeat_header","striped"), font_size = 12, full_width=F,position = "center") 
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE,dpi=2000,  fig.cap="\\textbf{Supplementary Fig. S1 | Percentage of present and missing values of the different traits.}"}
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=10, fig.width=10,  fig.cap="\\textbf{Supplementary Fig. S2 |  Phylogenetically informed principal component analysis for the species that did not have missing values.}"}
setwd("~/R_Projects/Reproductive Traits")
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=14, fig.width=28,  fig.cap="\\textbf{Supplementary Fig. S3 | Fitted posterior estimates of the number of visits made by bees including and exluding honey bees on the main axes of trait variation.} The superior (plots a, b and c) and lower (plots d, e and f) panels show visitation rates on the main axes of trait variation with and without \\emph{Apis mellifera}, respectively."} 
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=15, fig.width=12,fig.cap="\\textbf{Supplementary Fig. S4 | Regression tree analyses.} Comparison of the regression tree analyses between the full set of species and the subset of species with floral rewards for each plant species level metric. "}
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=10, fig.width=10,  fig.cap="\\textbf{Supplementary Fig. S5 | PC1 and PC3 of the phylogenetically informed principal component analysis with the full set of species.}"}
setwd("~/R_Projects/Reproductive Traits")
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=10, fig.width=10, fig.cap="\\textbf{Supplementary Fig. S6 | Phylogenetically informed principal component analysis for the subset of species with floral rewards.}"}
setwd("~/R_Projects/Reproductive Traits")
...
\newpage
\thispagestyle{empty}
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=19, fig.width=13, fig.cap="\\textbf{Supplementary Fig. S7 | Statistical comparison of the different categories of qualitative traits on the main two axes of trait variation with the full set of species.} Categories that differ significantly are denoted with a different letter."}
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=7, fig.width=28,  fig.cap="\\textbf{Supplementary Fig. S8 | Visitation rate of the main bee families in relation to the main axes of trait variation.} Fitted posterior estimates of the visitation rate of Andrenidae, Apidae, Colletidae, Halictidae and Megachilidae across PC1, PC2 and PC3. We omitted in the plotting area points over percentile 95 \\% percent due to extreme outliers."}
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=18, fig.width=26, fig.cap="\\textbf{Supplementary Fig. S9 | Plant species level metrics across the three main axes of trait variation.}", fig.pos= "h"}
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=18, fig.width=26, fig.cap="\\textbf{Supplementary Fig. S10 | Pearson correlation between the product of flower number-flower size and autonomous selfing level.}", fig.pos= "h"}
