...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=20, fig.width=10,out.width="85%",fig.align='center',fig.cap="\\textbf{Fig. S7.} Phylogenetic informed principal component analysis for the subset of species with floral rewards."}
...
all_spp_complete <- function(PC, x="PC1", y="PC2") {
  ...
  PCAloadings <- data.frame(Variables = rownames(PC$L), PC$L)
  plot <- plot + annotate("text", x = -(PCAloadings$PC1*c(4.6,4.95,4.8,5.4,5,4.2,5.8,3.4)+c(0,0,0,0,0,0,0,0)), y = -(PCAloadings$PC2*c(4.693,3.5,4,4,3,4.15,4.2,6.2)),
                          label = PCAloadings$Variables, color="black",fontface =2,size=4.5)
