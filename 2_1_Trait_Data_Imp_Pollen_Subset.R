...
t <- trait_filtered_2[c("Species_geonet","Order_all","Family_all","Genus_all","Species_all","Breeding_system","Compatibility_system","Autonomous_selfing_level",
                        ...
                        "Plant_height_mean_m","Pollen_per_flower", "Pollen_per_flower_species_level")]

#Levels are not perfect 
levels(factor(t$Pollen_per_flower_species_level))
...
t <- t[c("Species_geonet","Order_all","Family_all","Genus_all","Species_all","Breeding_system","Compatibility_system","Autonomous_selfing_level",
         "Autonomous_selfing_level_fruit_set", "Flower_morphology", "Flower_symmetry", "Flowers_per_plant",
         "Corolla_diameter_mean", "Corolla_length_mean", "Style_length", "Ovule_number", "life_form", "lifespan",
         "Plant_height_mean_m","Pollen_per_flower")]
...
t_pollen <- t[!is.na(t$Pollen_per_flower),]

#check missing data now
missing_data <- unlist(lapply(t_pollen, function(x) sum(is.na(x))))/nrow(t)*100
sort(missing_data[missing_data >= 0], decreasing=T)

#Calculate phylo distance
phylo <- as.data.frame(cbind(t_pollen$Family_all, t_pollen$Genus_all, t_pollen$Species_all))
...
#merge
dat_phylo <- merge(t_pollen, phylo_impute, by="Species_all")
...
#####
forest_imputed <- missForest(dat_phylo[,c(6:21)], maxiter = 10,mtry = 4, ntree = 200)
...
#7) SAVE DATA
########################################################################################################################################################
write.csv(forest_data, "Data/Csv/pollen_subset_imputed_trait_data.csv")
########################################################################################################################################################
########################################################################################################################################################
########################################################################################################################################################
