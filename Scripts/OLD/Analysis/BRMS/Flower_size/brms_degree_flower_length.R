...
str(all_df_2$degree)
all_df_2$net_id <- as.factor(all_df_2$net_id)
all_df_2$phylo <- as.factor(all_df_2$phylo)
all_df_2$degree <- as.factor(all_df_2$degree)

#NEGATIVE BINOMIAL DISTRIBUTION

m1 <- brm(degree ~ corolla_length_mean + (1|net_id) + (1|gr(phylo, cov = A)),
