...
####
##
# NETWORK ANALYSIS VISITS~FLOWER LENGTH
##
###
####
#####
...
all_df_2 <- all_df_2[complete.cases(all_df_2$corolla_length_mean),]
colnames(all_df_2) <- make.unique(names(all_df_2))
all_df_2$corolla_length_mean <- as.numeric(all_df_2$corolla_length_mean)

#NEGATIVE BINOMIAL DISTRIBUTION

m1 <- brm(visits ~ corolla_length_mean + (1|net_id) + (1|gr(phylo, cov = A)),
...
m2 <- brm(z_score_sum ~ log(corolla_length_mean) + (1|net_id) + (1|gr(phylo, cov = A)),
...
#SAVE MODELS
setwd("~/R_Projects/Reproductive traits") 
save(m1, file = "Data/Brms/Flower_size/brms_m1_visits_flower_length_negative_binomial.RData")
save(m2, file = "Data/Brms/Flower_size/brms_m1_z-scores_flower_length_student.RData")
save(all_df_2, file = "Data/Brms/Flower_size/brms_data_flower_length.RData")
