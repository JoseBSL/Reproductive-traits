...
####
##
# NETWORK ANALYSIS SPECIALIZATION ~ FLOWER MORPHOLOGY
##
###
####
#####
...
m1 <- brm(d ~ flower_morphology + (1|net_id) + (1|gr(phylo, cov = A)),
...
ggplot(data=p1[[1]], aes(x = reorder(flower_morphology, -d), y = d,color = ordered(flower_morphology))) +
  ...
  geom_point(data=p1[[1]], mapping=aes(x=flower_morphology, y=estimate__), color="black") + ylab("Specialization") + xlab("Selfing level")+
...
#SAVE MODELS
setwd("~/R_Projects/Reproductive traits") 
save(m1, file = "Data/Brms/Flower_morphology/brms_m1_degree_flower_morphology.RData")
save(all_df_3, file = "Data/Brms/Flower_morphology/brms_data_degree_m1_flower_morphology.RData")
