...
########################################################################################################################################################
d_a_1 <- subset(d_a, guild!="Other_insects" & guild!="Lizards" & guild!="Birds" & guild!="Bee") #Select just the main 6 functional groups
levels(factor(d_a_1$guild))
d_a_1$guild <- as.character(d_a_1$guild)
d_a_1$guild <- factor(d_a_1$guild, levels = c("Coleoptera", "Lepidoptera", "Non-bee-Hymenoptera",
...
analysis_1_no_bees <- brm((Interaction-1) ~ PC1*guild + PC2*guild + PC3*guild +(1|System/Id) + (1|gr(phylo, cov = A)),
...
summary(analysis_1_no_bees)
bayes_R2(analysis_1_no_bees)
loo_R2(analysis_1_no_bees)


marginal_effects(analysis_1_no_bees, effects = "PC1:guild")
pp_check(analysis_1_no_bees) +xlim(-50,200)+ylim(0,0.1)


#Save output
setwd("~/Dropbox/PhD/R/Chapter_2") #DROPBOX, files too large for github
saveRDS(analysis_1_no_bees, "results_analysis_1_no_bees.rds")
saveRDS(dat_analysis, "dat_analysis_results_analysis_1_no_bees.rds")
########################################################################################################################################################
#PLOT
########################################################################################################################################################
#Plot nicely PC1
...
p1 <- ggplot(ce_pc1[[1]], aes(x = -PC1, y = (estimate__+1), group=guild, colour=guild)) + geom_point(data = dat_analysis,
  ...
  theme_ms() + theme(legend.position = "none") + scale_color_manual(name="Functional groups",labels=c("Bees","Coleoptera", "Lepidoptera", "Non-bee-Hymenoptera",
   "Non-syrphids-diptera", "Syrphids"),values=c("orange", "black", "limegreen","#E7298A", "cyan4","blueviolet"),
    limits = c("Bees","Coleoptera", "Lepidoptera", "Non-bee-Hymenoptera","Non-syrphids-diptera", "Syrphids"))
...
#PREPARE POLL. FUNCTIONAL GROUPS FOR MODELLING
########################################################################################################################################################
d_a_1 <- subset(d_a, guild=="Bee") #Select just the main 6 functional groups
levels(factor(d_a_1$guild))
...
analysis_1_no_bees <- brm((Interaction-1) ~ PC1 + PC2 + PC3 +(1|System/Id) + (1|gr(phylo, cov = A)),
                          data = dat_analysis, family  = zero_inflated_negbinomial(),data2 = list(A = A_5), cores = 4,chains = 4, 
                          sample_prior = TRUE, warmup = 500, iter = 2000,
                          control = list(adapt_delta = 0.99))


analysis_1_just_bees <- analysis_1_no_bees

#Save output
setwd("~/Dropbox/PhD/R/Chapter_2") #DROPBOX, files too large for github
saveRDS(analysis_1_just_bees, "results_analysis_1_just_bees.rds")
saveRDS(dat_analysis, "dat_analysis_results_analysis_1_just_bees.rds")
########################################################################################################################################################
#PLOT
########################################################################################################################################################
#Plot nicely PC1
...
p1 <- ggplot(ce_pc1[[1]], aes(x = -PC1, y = (estimate__+1))) + geom_point(data = dat_analysis, aes(x = -PC1, y = Interaction),size = 0.75, alpha=0.5) + geom_line(size=0.8) + 
  ylim(0,quantile(dat_analysis$Interaction, 0.95)) + ylab("Number of visits")+ xlab("PC1")+
  theme_ms() + theme(legend.position = "none") 

ce_pc2 <- conditional_effects(analysis_1_just_bees, effects = "PC2",points=T) 

p2 <- ggplot(ce_pc2[[1]], aes(x = -PC2, y = (estimate__+1))) + geom_point(data = dat_analysis,aes(x = -PC2, y = Interaction),size = 0.75, alpha=0.5) + geom_line(size=0.8) + 
  ylim(0,quantile(dat_analysis$Interaction, 0.95)) + ylab("Number of visits")+ xlab("PC2")+
  theme_ms() + theme(legend.position = "none") 
...
p3 <- ggplot(ce_pc3[[1]], aes(x = -PC3, y = (estimate__+1))) + geom_point(data = dat_analysis,aes(x = -PC3, y = Interaction),size = 0.75, alpha=0.5) + geom_line(size=0.8) + 
  ylim(0,quantile(dat_analysis$Interaction, 0.95)) + ylab("Number of visits")+ xlab("PC3")+
  theme_ms() +theme(legend.position = "none") 
