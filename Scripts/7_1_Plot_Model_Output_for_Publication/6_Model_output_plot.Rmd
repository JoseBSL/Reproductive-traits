...
m_5_clust_zero_neg_hclust <- readRDS("model_forest_data.RDS")
...
t <- ggplot(ce_1[[1]], aes(x = Clusters, y = Interaction, colour = as.factor(guild), group = 
  ...
  theme_bw()+ ylab("Nº of visits per plant taxa") + xlab("Plant functional groups")+
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=9, fig.width=13}
library(ggplot2)
library(brms)
library(png)
library(grid)
