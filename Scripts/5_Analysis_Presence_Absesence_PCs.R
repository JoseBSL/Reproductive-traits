...
analysis_1 <- brm(Interaction  ~ PC1*guild + PC2*guild + PC3*guild +(1|System/Id) + (1|gr(phylo, cov = A)),
...
marginal_effects(analysis_1)
marginal_effects(analysis_1, effects = "PC2:guild")
marginal_effects(analysis_1, effects = "PC3:guild")
...
p1 <- ggplot(ce_pc1[[1]], aes(x = -PC1, y = (estimate__), group=guild, colour=guild)) + geom_point(data = dat_analysis,
...
p2 <- ggplot(ce_pc2[[1]], aes(x = -PC2, y = (estimate__), group=guild, colour=guild)) + geom_point(data = dat_analysis,
...
p3 <- ggplot(ce_pc3[[1]], aes(x = -PC3, y = (estimate__), group=guild, colour=guild)) + geom_point(data = dat_analysis,
