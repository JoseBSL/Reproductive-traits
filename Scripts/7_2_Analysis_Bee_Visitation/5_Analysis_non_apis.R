...
d_5 <- read.csv("Data/Csv/long_format_trait_data_non_apis_5_clusters.csv", row.names = 1)
########################################################################################################################################################
...
#5 clusters hclust
m_5_clust_zero_neg_hclust_non_apis <- brm((Interaction-1) ~ guild*Clusters + (1|Id) + (1|gr(phylo, cov = A)),
                                 data = d_5_1, family  = zero_inflated_negbinomial(),data2 = list(A = A_5), cores = 4,chains = 4, 
                                 sample_prior = TRUE, warmup = 500, iter = 1500,
                                 control = list(adapt_delta = 0.99)) 

marginal_effects(m_5_clust_zero_neg_hclust_non_apis, effects = "Clusters:guild")
pp_check(m_5_clust_zero_neg_hclust_non_apis) +xlim(-50,200)+ylim(0,0.1)
pp_check(m_5_clust_zero_neg_hclust_non_apis, type='violin_grouped',group="Clusters")+ylim(-4,4)
pp_check(m_5_clust_zero_neg_hclust_non_apis, type='violin_grouped',group="guild")+ylim(-4,4)
saveRDS(m_5_clust_zero_neg_hclust_non_apis, "m_5_clust_zero_neg_hclust_non_apis")



m_5_clust_zero_neg_hclust_non_apis <- readRDS("m_5_clust_zero_neg_hclust_non_apis")
...
#############
ce_1 <- conditional_effects(m_5_clust_zero_neg_hclust_non_apis, effects = "Clusters:guild",points=T) 
