...
data_bee = data_analysis %>% 
  mutate(guild = case_when(guild=="Non-syrphids-diptera" ~ "Non-syrphids-diptera", 
                           guild!="Non-syrphids-diptera" ~ "Other")) %>% 
  filter(guild == "Non-syrphids-diptera") %>% 
...
#Save plot
p = PCbiplot(PC)
saveRDS(p, "Data/RData/Non_syrphid_diptera_PCA.rds")
