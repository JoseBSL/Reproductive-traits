...
data_bee = data_analysis %>% 
  mutate(guild = case_when(guild=="Syrphids" ~ "Syrphids", 
                           guild!="Syrphids" ~ "Other")) %>% 
  filter(guild == "Syrphids") %>% 
...
dat_cleaning_5 = dat_cleaning_5 %>% 
  mutate(guild = case_when(guild=="Syrphids" ~ "Syrphids", 
                           is.na(guild) ~ "Other"))  %>% 
  mutate(guild = factor(guild, levels = c("Syrphids","Other")))
...
PCbiplot <- function(PC, x="PC1", y="PC2") {
  ...
  plot <- plot + theme_ms() +ylim(-4,4) + xlim(-4,4) +  theme(legend.position = c(0.11, 0.130)) +
    ggtitle("(f) Syrphids") + xlab("Flower number - Flower size axis")  + ylab(NULL)
...
#Save plot
p = PCbiplot(PC)
saveRDS(p, "Data/RData/Syrphids_PCA.rds")