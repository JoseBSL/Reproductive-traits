...
data_bee = data_analysis %>% 
  mutate(guild = case_when(guild=="Bee" ~ "Bee", 
  guild!="Bee" ~ "Other")) %>% 
  filter(guild == "Bee") %>% 
...
PCbiplot <- function(PC, x="PC1", y="PC2") {
  ...
  plot <- plot + theme_ms() +ylim(-4,4) + xlim(-4,4) +  theme(legend.position = c(0.220, 0.130)) +ggtitle("(a) Bees") + xlab(NULL)  + ylab("Autonomous selfing - Floral display axis")
...
#Save plot
p = PCbiplot(PC)
saveRDS(p, "Data/RData/Bee_PCA.rds")
