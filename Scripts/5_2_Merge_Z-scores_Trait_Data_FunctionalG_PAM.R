...
pam_d <- read.csv("Data/Csv/imputed_trait_data_pam_14_clusters.csv") 
...
colnames(pam_d)[1] <-"Species_all"
#convert to chracters
pam_d$Species_all <- as.character(pam_d$Species_all)
...
pam_d$Species_all[pam_d$Species_all=="NA"]<- NA

#remove NA'S
long_format_trait_data <- long_format_trait_data[!is.na(long_format_trait_data$Species_all),]
pam_d <- pam_d[!is.na(pam_d$Species_all),]

#merge
quantitative_networks_Z_scores_with_traits_and_clusters <- merge(long_format_trait_data,pam_d, by="Species_all", all.x  = T) #now data is ready for analysis
...
write.csv(quantitative_networks_Z_scores_with_traits_and_clusters, "Data/Csv/quantitative_networks_Z_scores_with_traits_and_14_clusters.csv")
