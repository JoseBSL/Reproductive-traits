...
dat <- as.data.frame(phyl_pca_forest$S)
...
saveRDS(phyl_pca_forest, "Data/RData/phyl_pca_forest_all_values.rds")
...
plot <- plot + geom_segment(data=datapc, aes(x=0, y=0, xend=v1, yend=v2),size=0.6, arrow=arrow(length=unit(0,"cm")),linetype=2, alpha=0.8, color="black")
 ...
 plot <- plot + annotate("text", x = (PCAloadings$PC1*c(3.5,4,4,4,4,4)), y = (PCAloadings$PC2*c(3,4,4,4,4,4)),label = PCAloadings$Variables, color="black",size=5) 
 # 
 # #CHANGE THEME
