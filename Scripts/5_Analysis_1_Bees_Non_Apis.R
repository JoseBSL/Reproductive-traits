...
d_a <- readRDS("Data/RData/data_analysis_1_bees_non_apis.rds") #ALL SPP
########################################################################################################################################################
...
analysis_1_bee_families_non_apis <- brm((Interaction-1) ~ PC1*bee_family + PC2*bee_family + PC3*bee_family +(1|System/Id) + (1|gr(phylo, cov = A)),
...
summary(analysis_1_bee_families_non_apis)
bayes_R2(analysis_1_bee_families_non_apis)
loo_R2(analysis_1_bee_families_non_apis)


marginal_effects(analysis_1_bee_families_non_apis, effects = "PC1:bee_family")
marginal_effects(analysis_1_bee_families_non_apis, effects = "PC2:bee_family")
marginal_effects(analysis_1_bee_families_non_apis, effects = "PC3:bee_family")


pp_check(analysis_1_bee_families_non_apis) +xlim(-50,200)+ylim(0,0.1)
pp_check(analysis_1_bee_families_non_apis, type='violin_grouped',group="Clusters")+ylim(-4,4)
pp_check(analysis_1_bee_families_non_apis, type='violin_grouped',group="guild")+ylim(-4,4)

#Save data
#Save output
setwd("~/Dropbox/PhD/R/Chapter_2") #DROPBOX, files too large for github
saveRDS(analysis_1_bee_families_non_apis, "results_analysis_1_bee_families_non_apis.rds")
saveRDS(dat_analysis, "dat_analysis_results_analysis_1_bee_families_non_apis.rds")
########################################################################################################################################################
#PLOT
########################################################################################################################################################
#Plot nicely PC1
...
ce_pc1 <- conditional_effects(analysis_1_bee_families_non_apis, effects = "PC1:bee_family",points=T) 
...
#Plot nicely PC2
ce_pc2 <- conditional_effects(analysis_1_bee_families_non_apis, effects = "PC2:bee_family",points=T) 
...
#Plot nicely PC3
ce_pc3 <- conditional_effects(analysis_1_bee_families_non_apis, effects = "PC3:bee_family",points=T) 
...
d_a_1 <- subset(d_a, bee_family!="Melittidae" & bee_family!="Megachilidae" & bee_family!="Andrenidae") #Select just the main functional groups
levels(factor(d_a_1$bee_family))
#save on this dataframe
dat_analysis <- d_a_1
########################################################################################################################################################
...
analysis_1_bee_families <- brm((Interaction-1) ~ PC1*bee_family + PC2*bee_family + PC3*bee_family +(1|System/Id) + (1|gr(phylo, cov = A)),
                               data = dat_analysis, family  = zero_inflated_negbinomial(),data2 = list(A = A_5), cores = 4,chains = 4, 
                               sample_prior = TRUE, warmup = 500, iter = 2000,
                               control = list(adapt_delta = 0.99))


ce_pc1 <- conditional_effects(analysis_1_bee_families, effects = "PC1:bee_family",points=T) 
...
p1 <- ggplot(ce_pc1[[1]], aes(x = -PC1, y = (estimate__+1), group=bee_family, colour=bee_family)) + geom_point(data = dat_analysis,
  ...
  theme_ms() + theme(legend.position = "none") + scale_color_manual(name="Functional groups",values=c("orange", "black", "limegreen","#E7298A", "cyan4","blueviolet"))
#Plot nicely PC2
ce_pc2 <- conditional_effects(analysis_1_bee_families, effects = "PC2:bee_family",points=T) 
...
#Plot nicely PC3
ce_pc3 <- conditional_effects(analysis_1_bee_families, effects = "PC3:bee_family",points=T) 
