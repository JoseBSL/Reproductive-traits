...
####
##
# NETWORK ANALYSIS DEGREE ~ COROLLA DIAMETER
##
###
####
#####
...
##########################################################
#4 Model1 (m1) VISITS~FLOWER LENGTH
##########################################################
...
m1 <- brm(degree ~ corolla_diameter_mean + (1|net_id) + (1|gr(phylo, cov = A)),sample_prior = TRUE,
          data = all_df_2, family = negbinomial(),data2 = list(A = A), cores = 4, warmup = 500, iter = 1500,save_all_pars=T,
...
ggplot(data=p1[[1]], aes(x = log(corolla_diameter_mean), y = degree)) +
...
#SAVE MODELS
setwd("~/R_Projects/Reproductive traits") 
save(m1, file = "Data/Brms/Flower_size/brms_m1_degree_flower_diameter_negative_binomial.RData")
save(all_df_2, file = "Data/Brms/Flower_size/brms_data_degree_flower_diameter.RData")



#try also with the scaled flower size and see for differences between models

all_df_new <- all_df_2 %>%
  group_by(net_id) %>%
  mutate(corolla_diameter_mean = scale(corolla_diameter_mean))

all_df_new$corolla_diameter_mean

m1 <- brm(degree ~ corolla_diameter_mean + (1|net_id) + (1|gr(phylo, cov = A)),
          data = all_df_new, family = negbinomial(),data2 = list(A = A), cores = 4, warmup = 500, iter = 1500,save_all_pars=T,
          control = list(adapt_delta = 0.99))
...
library(performance)
r2_bayes(m1)
ggplot(data=p1[[1]], aes(x = corolla_diameter_mean, y = degree)) +
  geom_point(data = all_df_new,alpha = 1/4) + 
  scale_fill_brewer(palette = "Greys") +
  scale_color_brewer(palette = "Set2") + theme_bw() + geom_smooth(data = p1[[1]],
...
#SAVE MODELS
setwd("~/R_Projects/Reproductive traits") 
save(m1, file = "Data/Brms/Flower_size/brms_m1_degree_flower_length_negative_binomial.RData")
save(all_df_2, file = "Data/Brms/Flower_size/brms_data_degree_flower_length.RData")
