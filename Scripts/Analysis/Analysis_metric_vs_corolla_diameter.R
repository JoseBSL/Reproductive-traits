...
model_5 <- lmer(closeness ~ Corolla_diameter_mean + (1|Id) , data = all_df)
...
mydf <- ggpredict(model_5, terms = c("Corolla_diameter_mean"))
ggplot(mydf, aes(x = x, y = predicted)) + theme_ds4psy() + theme(legend.position = "none") + xlab("Corolla diameter (mm)")+ ylab("Predicted d")+geom_point(data= all_tf, aes(x =Corolla_diameter_mean , y = pred))+
  geom_line( alpha = 1)+ geom_ribbon(aes(x = x,ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.1, colour = NA)


saveRDS(all_tf, "Data/RData/data_plot_visits_corolla_diameter_5.rds")
saveRDS(model_5, "Data/RData/model_5_visits_corolla_diameter_5.rds")
saveRDS(pred, "Data/RData/pred_plot_visits_corolla_diameter_5.rds")
