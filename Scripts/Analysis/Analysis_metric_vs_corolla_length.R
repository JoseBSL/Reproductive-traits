...
make.true.NA <- function(x) if(is.character(x)||is.factor(x)){
  is.na(x) <- x=="NA"; x} else {
    x}
all_df$Corolla_length_mean <- make.true.NA(all_df$Corolla_length_mean)
all_df <- all_df[complete.cases(all_df$Corolla_length_mean),]
colnames(all_df) <- make.unique(names(all_df))
all_df$Id <- as.factor(all_df$Id)
all_df$Corolla_length_mean <- as.numeric(all_df$Corolla_length_mean)
all_df$Visits <- as.numeric(all_df$Visits)
...
model_1 <- mixed_model(Visits ~ Corolla_length_mean, random = ~ 1 | Id, data = all_df,
...
ggplot(mydf, aes(x = x, y = predicted)) + theme_ds4psy() + theme(legend.position = "none") + xlab("Corolla length (mm)")+ ylab("Predicted visits")+geom_point(data= all_tf, aes(x =Corolla_length_mean , y = pred))+
  geom_line( alpha = 1)+ geom_ribbon(aes(x = x,ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.1, colour = NA)


saveRDS(model_1, "Data/RData/model_1_visits_corolla_length.rds")
saveRDS(all_tf, "Data/RData/data_plot_visits_corolla_length.rds")
...
library(multcomp)
hist(all_df$d)
model_2 <- lmer(d ~Corolla_length_mean + (1|Id) , data = all_df)
summary(model_2)
...
ggplot(mydf, aes(x = x, y = predicted)) + theme_ds4psy() + theme(legend.position = "none") + xlab("Corolla length")+ ylab("Predicted d")+geom_point(data= all_tf, aes(x =Corolla_length_mean , y = pred))+
  geom_line( alpha = 1, fill=c("blue", "red", "green"))+ geom_ribbon(aes(x = x,ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.1, colour = NA)


saveRDS(all_tf, "Data/RData/data_plot_visits_corolla_length_2.rds")
saveRDS(model_2, "Data/RData/model_2_visits_corolla_length_2.rds")
saveRDS(pred, "Data/RData/pred_plot_visits_corolla_length_2.rds")
...
model_3 <- mixed_model(degree ~ Corolla_length_mean, random = ~ 1 | Id, data = all_df,
                       family = negative.binomial())
...
ggplot(mydf, aes(x = x, y = predicted)) + theme_ds4psy() + theme(legend.position = "none") + xlab("Corolla length")+ ylab("Predicted degree")+ geom_point(data= all_tf, aes(x =Corolla_length_mean , y = pred))+
  geom_line( alpha = 1, fill=c("blue", "red", "green"))+ geom_ribbon(aes(x = x,ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.1, colour = NA)


saveRDS(all_tf, "Data/RData/data_plot_visits_corolla_length_3.rds")
