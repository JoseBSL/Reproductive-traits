...
#TRY FIRST THE (1) NEGATIVE BINOMIAL DISTRIBUTION
m3 <- brm(visits ~ autonomous_selfing_level_fruit_set + (1|net_id) + (1|gr(phylo, cov = A)),
...
summary(m3)
pp_check(m3) + xlim(-100,2000)+ylim(0,0.02)
...
m3.1 <- brm(log(visits) ~ autonomous_selfing_level_fruit_set + (1|net_id) + (1|gr(phylo, cov = A)),
...
summary(m3.1)
pp_check(m3.1) 
c_e.3.1 <- conditional_effects(m3.1)
p3.1 <- plot(c_e.3.1, points=T,plot = FALSE)[[1]]
bayes_R2(m3.1)

#PLOT OUTPUT

ggplot(data=p3.1[[1]], aes(x = autonomous_selfing_level_fruit_set, y = log(visits))) +
  geom_point(data = all_df_3,alpha = 1/4) + 
  scale_fill_brewer(palette = "Greys") +
...
summary(m4)
pp_check(m4) +xlim(-10,10)
c_e.4 <- conditional_effects(m4)
p4 <- plot(c_e.4, points=T,plot = FALSE)[[1]]
bayes_R2(m4)
