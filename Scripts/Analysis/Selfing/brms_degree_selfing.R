...
m1 <- brm(degree ~ autonomous_selfing_level + (1|net_id) + (1|gr(phylo, cov = A)),
...
#TRY FIRST THE (1) NEGATIVE BINOMIAL DISTRIBUTION
#Run model
m2 <- brm(degree ~ autonomous_selfing_level + (1|net_id) + (1|gr(phylo, cov = A)),
...
summary(m2)
pp_check(m2) + xlim(-200,200)+ylim(0,0.2)
...
ggplot(data=p2[[1]], aes(x = autonomous_selfing_level, y = degree,color = ordered(autonomous_selfing_level))) +
  geom_point(data = all_df_3,alpha = 1/4) + 
  scale_fill_brewer(palette = "Greys") +
  scale_color_brewer(palette = "Set2") + theme_bw() +
  geom_errorbar(data=p2[[1]],mapping=aes(x=autonomous_selfing_level, ymin=lower__, ymax=upper__), width=.1, color="black")+
  geom_point(data=p2[[1]], mapping=aes(x=autonomous_selfing_level, y=estimate__), color="black") + ylab("Visits") + xlab("Selfing level")+
  theme(legend.position = "none")

#TRY NOW THE (2) SKEW NORMAL DISTRIBUTION

######################################################################
#4 Model3 (m3) VISITS~AUTONOMOUS SELFING LEVEL JUST QUANTITATIVE DATA
######################################################################

#convert to numeric 
all_df_3$autonomous_selfing_level_fruit_set <- as.numeric(all_df_3$autonomous_selfing_level_fruit_set)
all_df_3$autonomous_selfing_level_fruit_set <- make.true.NA(all_df_3$autonomous_selfing_level_fruit_set)
all_df_3 <- all_df_3[complete.cases(all_df_3$autonomous_selfing_level_fruit_set),]


#TRY FIRST THE (1) NEGATIVE BINOMIAL DISTRIBUTION
m3 <- brm(degree ~ autonomous_selfing_level_fruit_set + (1|net_id) + (1|gr(phylo, cov = A)),
          data = all_df_3, family = negbinomial(),data2 = list(A = A), cores = 4,
          sample_prior = TRUE, warmup = 500, iter = 1500,save_all_pars=T,
          control = list(adapt_delta = 0.99))
...
summary(m3)
pp_check(m3) + xlim(-200,200)+ylim(0,0.2)
...
#SAVE MODELS
setwd("~/R_Projects/Reproductive traits") 
save(m1, file = "Data/Brms/Selfing/brms_m1_degree_selfing_quant_and_quali_negative_binomial.RData")
save(m2, file = "Data/Brms/Selfing/brms_m2_degree_selfing_quant_as_quali_negative_binomial.RData")
save(m3, file = "Data/Brms/Selfing/brms_m3_degree_selfing_quant_negative_binomial.RData")
save(all_df_3, file = "Data/Brms/Selfing/brms_data_degree_m1_selfing.RData")
save(all_df_2, file = "Data/Brms/Selfing/brms_data_degree_m2_m3_selfing.RData")
