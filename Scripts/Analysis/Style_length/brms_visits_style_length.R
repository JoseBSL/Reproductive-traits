...
####
##
# NETWORK ANALYSIS VISITS~STLE LENGTH
##
###
####
#####
...
##########################################################
#4 Model1 (m1) VISITS~STYLE LENGTH
##########################################################
...
all_df_2 <- all_df_2[complete.cases(all_df_2$style_length_mm),]
colnames(all_df_2) <- make.unique(names(all_df_2))
all_df_2$style_length_mm <- as.numeric(all_df_2$style_length_mm)

#NEGATIVE BINOMIAL DISTRIBUTION

m1 <- brm(visits ~ style_length_mm + (1|net_id) + (1|gr(phylo, cov = A)),
...
m2 <- brm(z_score_sum ~ style_length_mm + (1|net_id) + (1|gr(phylo, cov = A)),
...
#SAVE MODELS
setwd("~/R_Projects/Reproductive traits") 
save(m1, file = "Data/Brms/Style_length/brms_m1_visits_style_length_negative_binomial.RData")
save(m2, file = "Data/Brms/Style_length/brms_m1_z-scores_style_length_student.RData")
save(all_df_2, file = "Data/Brms/Style_length/brms_data_style_length.RData")
