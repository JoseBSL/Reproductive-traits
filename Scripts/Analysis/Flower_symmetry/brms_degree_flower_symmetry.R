...
m1 <- brm(degree ~ flower_symmetry + (1|net_id) + (1|gr(phylo, cov = A)),
...
ggplot(data=p1[[1]], aes(x = flower_symmetry, y = degree,color = ordered(flower_symmetry))) +
  ...
  geom_point(data=p1[[1]], mapping=aes(x=flower_symmetry, y=(estimate__)), color="black") + ylab("log(Visits)") + xlab("Flower symmetry")+
  theme(legend.position = "none")


#SAVE MODELS
setwd("~/R_Projects/Reproductive traits") 
save(m1, file = "Data/Brms/Flower_symmetry/brms_m1_degree_flower_symmetry_negative_binomial.RData")
save(all_df_3, file = "Data/Brms/Flower_symmetry/brms_data_degree_flower_symmetry.RData")
