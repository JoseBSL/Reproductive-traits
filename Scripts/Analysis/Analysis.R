...
#####################################
###
#### MODEL 4 Normalise degree ~ COMPATIBILITY
###
#####################################
library(lme4)
library(DHARMa)
library(emmeans)
library(multcomp)
model_4 <- lmer(normalised.degree ~ Compatibility + (1|Id) , data = all_df)
...
#####################################
###
#### MODEL 5 closeness ~ COMPATIBILITY
###
#####################################
library(lme4)
library(DHARMa)
...
model_5 <- lmer(closeness ~ Compatibility + (1|Id) , data = all_df)
fit<- simulateResiduals(fittedModel = model_5, plot = T)
plot(fit)
#Looks good

#Check differences in d between compatibility modes
lsm = lsmeans(model_5, "Compatibility", type = "response")
pairs(lsm)
CLD <- cld(lsm,alpha=0.05,adjust="tukey")
...
model_6 <- lmer(betweenness ~ Compatibility + (1|Id) , data = all_df)
fit<- simulateResiduals(fittedModel = model_6, plot = T)
plot(fit)
#Super ugly as expected
#Let's look for another distribution, many zeross!
...
model_6 <- mixed_model(betweenness ~ Compatibility, random = ~ 1 | Id, data = all_df,
                         family = poisson())
summary(model_6)
resids_plot(model_6, all_df$betweenness)
#Now doesn't look to bad (Poisson)
#Let's see zi.poisson
model_6_1 <- mixed_model(betweenness ~ Compatibility, random = ~ 1 | Id, data = all_df,
                       family = zi.poisson(), zi_fixed= ~1,iter_EM = 0)
resids_plot(model_6_1, all_df$betweenness)

model_6_2 <- mixed_model(betweenness ~ Compatibility, random = ~ 1 | Id, data = all_df,
                         family = zi.negative.binomial(), zi_fixed= ~1,iter_EM = 0)
resids_plot(model_6_2, all_df$betweenness)
summary(model_6_2)

#Does not change too much, I stick with a zero inflated negative binomial
#To keep constancy between models


library(emmeans)
library(multcomp)
#Analyse data and plot it
lsm = lsmeans(model_6, "Compatibility", type = "response")
pairs(lsm)
CLD <- cld(lsm,alpha=0.05,adjust="tukey")
...
#Predict values
all_tf <- all_df
preds <- predict(model_6, newdata = all_tf,
                 type = "subject_specific",
                 se.fit = TRUE, return_newdata = TRUE)
...
#Plot predicted values and mean predicted value with CI
ggplot(preds,aes(x= Compatibility,y= pred, color=Compatibility)) +theme()+
  geom_point(size   = 4, alpha=0.2, colour="lightblue") + geom_point(aes(y= mean, color=Compatibility),Compatibility  = 2,size   = 4,colour="steelblue", alpha=1) +
  geom_errorbar(aes(ymin  =  lower,ymax  =  upper),width =  0.08,size =  0.7, colour="steelblue")
