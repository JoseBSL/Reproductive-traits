...
#1) READ DATA
########################################################################################################################################################
d_a <- readRDS("Data/RData/data_analysis_1_non_apis.rds") #ALL SPP
########################################################################################################################################################
...
analysis_1_non_apis <- brm((Interaction-1) ~ PC1*guild + PC2*guild + PC3*guild +(1|System/Id) + (1|gr(phylo, cov = A)),
...
summary(analysis_1_non_apis)
bayes_R2(analysis_1_non_apis)
loo_R2(analysis_1_non_apis)


marginal_effects(analysis_1_non_apis, effects = "PC1:guild")
pp_check(analysis_1_non_apis) +xlim(-50,200)+ylim(0,0.1)
pp_check(analysis_1_non_apis, type='violin_grouped',group="Clusters")+ylim(-4,4)
pp_check(analysis_1_non_apis, type='violin_grouped',group="guild")+ylim(-4,4)
...
saveRDS(analysis_1_non_apis, "results_analysis_1_non_apis.rds")
saveRDS(dat_analysis, "dat_analysis_results_analysis_1_non_apis.rds")
########################################################################################################################################################
#PLOT
########################################################################################################################################################
#Plot nicely PC1
...
ce_pc1 <- conditional_effects(analysis_1_non_apis, effects = "PC1:guild",points=T) 
...
#Plot nicely PC2
ce_pc2 <- conditional_effects(analysis_1_non_apis, effects = "PC2:guild",points=T) 
...
#Plot nicely PC3
ce_pc3 <- conditional_effects(analysis_1_non_apis, effects = "PC3:guild",points=T)
